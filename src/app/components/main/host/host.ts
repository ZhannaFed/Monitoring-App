...
export class Host implements OnInit {
  ...
  */
  ...
  // TEST HOST DETAILS DATA ============================
  private applyMockHostDetails(): void {
    const detail = MOCK_HOST_DETAILS[this.id];
    if (!detail) {
      this.hostStatus = false;
      this.diskSpaceTransformed = [];
      this.cpuUtil = [];
      this.memoryUtil = [];
      this.opSystemInfo = [];
      this.hostname = [];
    ...
    this.hostStatus = detail.isAvailable;
    this.diskSpaceTransformed = detail.disks;
    this.cpuUtil = [{ lastvalue: detail.cpu }];
